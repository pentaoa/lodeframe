...
final class ShaderPackGeometryRenderer implements AutoCloseable {
    ...
    private final Set<RenderPipeline> ownedPipelines = Collections.newSetFromMap(new IdentityHashMap<>());
    ...
    ShaderPackGeometryRenderer(
        ...
        this.vertexUniforms = program.vertex().uniforms().isEmpty()
                ? null
                : new ShaderPackUniformState(device, program.vertex().uniforms(), label + " vertex uniforms");
        this.fragmentUniforms = program.fragment().uniforms().isEmpty()
                ? null
                : new ShaderPackUniformState(device, program.fragment().uniforms(), label + " fragment uniforms");
    }
    ...
    RenderPipeline override(final RenderPipeline base) {
        return this.overrides.computeIfAbsent(base, pipeline -> {
            ...
            if (!compiled.isValid()) {
                ...
                throw new IllegalStateException("Metal rejected " + this.label + " program " + this.program.program().key());
            }
            this.ownedPipelines.add(result);
            return result;
        });
    }

    boolean owns(final RenderPipeline pipeline) {
        return this.ownedPipelines.contains(pipeline);
    }
    ...
    void bindResources(final MetalRenderPass pass) {
        if (this.vertexUniforms != null) {
            this.vertexUniforms.bind(pass, LegacyFullscreenTransformer.uniformBlockName(ShaderStage.VERTEX));
        }
        if (this.fragmentUniforms != null) {
    ...
    @Override
    public void close() {
        for (RenderPipeline pipeline : this.ownedPipelines) {
